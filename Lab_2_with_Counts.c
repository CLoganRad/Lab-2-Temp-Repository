...
volatile char count_switch_press1 = 0;
volatile char count_switch_flag1 = 0;
volatile char count_switch_press2 = 0;
volatile char count_switch_flag2 = 0;
...
int main(void)
{
	...
	while(1)
    {
		   ...
		   // The argument of the If is TRUE only if the switch is closed (and Pin3 is a high voltage), in which case we want to turn on LED 1.
		{
			...
			if (count_switch_flag1 == 0)
			{
				count_switch_press1++;
			count_switch_flag1 = 1;
			}
			...
			// Same as above but pin PD4 is the target.
		{
			...
			if (count_switch_flag2 == 0)
			{
				count_switch_press2++;
			count_switch_flag2 = 1;
			}
